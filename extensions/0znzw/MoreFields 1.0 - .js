...
(function (Scratch) {
  ...
  function _fixColours(doText, col1, textColor) {
    ...
    if (doText) _setCssNattr(textNode, 'fill', textColor ?? '#FFFFFF');
  }
  ...
  vm.addListener('EXTENSION_FIELD_ADDED', (fieldInfo) => {
    if (!!window.ScratchBlocks) ScratchBlocks.Field.register(fieldInfo.name, fieldInfo.implementation);
  });
  ...
  class FieldTextarea extends Blockly.FieldTextInput {
  ...
  class FieldInlineTextarea extends Blockly.Field {
    constructor(opt_value) {
  ...
  class FieldSnapBoolean extends Blockly.Field {
    constructor(opt_value) {
  ...
  class FieldInlineSlider extends Blockly.FieldNumber {
    constructor(opt_value) {
  ...
  class FieldHiddenTextInput extends Blockly.FieldTextInput {
    constructor(opt_value) {
    ...
    init(...initArgs) {
      ...
      this.textNode__ = this.sourceBlock_.svgPath_.parentNode.querySelector('g.blocklyEditableText text');
      _fixColours.call(this, true, this.sourceBlock_.parentBlock_.colour_, this.sourceBlock_.parentBlock_.colour_);
    }
    showEditor_(...showArgs) {
      _delCssNattr(this.textNode__, 'fill');
      Blockly.FieldTextInput.prototype.showEditor_.call(this, ...showArgs);
    }
  }
  class FieldInlineDate extends Blockly.Field {
  ...
  class FieldFileInput extends Blockly.Field {
    constructor(opt_value) {
  ...
  class FieldInlineDoom extends Blockly.Field {
    showEditor_() {
    }
    constructor(opt_value) {
  ...
  // Adding customFieldTypes
  customFieldTypes[Scratch.ArgumentType.TEXTAREA] = {
    output: 'String',
    color1: '#9566d3',
    outputShape: 2,
    implementation: {
      fromJson: () => new FieldTextarea()
    }
  };
  customFieldTypes[Scratch.ArgumentType.INLINETEXTAREA] = {
    ...
    outputShape: 2,
    implementation: {
      fromJson: () => new FieldInlineTextarea()
    }
  };
  ...
  customFieldTypes[Scratch.ArgumentType.INLINESLIDER] = {
    ...
    outputShape: 3,
    implementation: {
      fromJson: () => new FieldInlineSlider()
    }
  };
  customFieldTypes[Scratch.ArgumentType.HIDDENSTRING] = {
    ...
    outputShape: 2,
    implementation: {
      fromJson: () => new FieldHiddenTextInput()
    }
  };
  ...
  // A CRAP TON of warning related stuff.
  // bc I dont wanna hear people say their projects got corrupted,
  // from going into the editor.
  // (IF your in the code you cant say anything cause u saw dis)
  const openedEditorInitially = (ReduxStore?.getState?.()?.scratchGui?.mode?.hasEverEnteredEditor) ?? false;
  let skipWarning = false;
  function editorWarning(usingButton) {
    if (!(usingButton ?? false) && skipWarning) return;
    alert(`${(openedEditorInitially ? 'If you' : 'You')} did not open this project in the editor initially please read the rest of this.
If so DO NOT save or run your project in this state, immediantly close the tab and load the project via the editor,
failure to due so can corrupt your project and crash the editor.
I will display a prompt to kill the project for you.`);
    if (confirm(`This will kill the project without saving or running anything and reload to the editor.
If this was an accident please press "cancel" otherwise press ok.`)) {
      ...
      alert(`Proceed at your OWN risk, I am not liable for any project damage.
You WILL NOT recive this warning again.
Well unless you use the button but you get what I mean.`);
    }
    skipWarning = true;
  }
  if (!!window.ReduxStore) {
    // Garbo said this is the best way :sob:
    ReduxStore.subscribe(() => {
      const scratchGui = ReduxStore.getState().scratchGui;
      const inEditor = scratchGui.mode.hasEverEnteredEditor && !scratchGui.mode.isPlayerOnly;
      if (inEditor && !openedEditorInitially) editorWarning();
    });
  }
  ...
  class extension {
        ...
        blocks: [
          {
            ...
            xml: '<sep gap="6"/><label text="If you did not open this,"/><sep gap="-12"/><label text="in the editor initially please"/><sep gap="-12"/><label text="press the button below."/><sep gap="-12"/>'
          },
          {
    ...
    editorWarning() {
      // Im not doing this.editorWarning so this is not recursive dw.
      editorWarning(true);
    }
    textarea(args) {
      return args.TEXT;
