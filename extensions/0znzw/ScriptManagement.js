...
Finished:
  set block json
  save / set data to or from block
  save / set data to or from thread
  stop script / block
  start script / block
  toggle block / stack clicks
  thread management
  project json (duh)
  blocks in thread
  get scripts / block

Added on the fly (and finished):
  get closest C block to block

Cancled:
  F: get blocks in sprite. R: replaced with "get scripts in sprite"
  F: stack cloning (interpreter only) R: To buggy + no compiler support
*/
(function(Scratch) {
    'use strict';
    ...
    /* function by @0znzw, if used please keep this comment */
    ...
    function label(text) { return { blockType: 'label', text } }
    function button(text, func) { return { blockType: 'button', text, func } }
    //@ts-ignore
  ...
  Warning Prefix labels: 
    ...
    pc: Project corruption`);
      ...
      getBlockViaID(args, util) {
        ...
        } catch { return '{"error":"invalid block"}' }
      }
      get_myself_id() {
