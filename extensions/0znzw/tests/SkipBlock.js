...
(function (Scratch) {
    (function () {
      ...
      this.exports.Thread = Thread;
      this.exports._StackFrame = _StackFrame;
    }).call(window.vm);
    const vm = Scratch.vm;
    function cloneThread(thread) {
      //@ts-ignore
      const nThread = new vm.exports.Thread(thread.topBlock);
      //nThread.generator = thread.generator;
    ...
    class extension {
      getInfo() {
        return {
          ...
          blocks: [
            {
              opcode: 'skip',
              text: ['while [COND]', 'when not: stop immediately'],
      ...
      async skip(args, util, json) {
        let thread = util.thread;
        const blockContainer = util.target.blocks;
        const myId = thread.peekStack();
        let cThread = cloneThread(thread);
        cThread.topBlock = blockContainer.getNextBlock(myId);
        cThread.stack = [cThread.topBlock];
        if (args.COND) util.startBranch(1, true);
        vm.runtime.threads.push(cThread);
        await until(_ => !vm.runtime.isActiveThread(cThread));
        let uThread = cloneThread(cThread);
        uThread.topBlock = thread.topBlock;
        uThread.stack = structuredClone(thread.stack);
        thread.justReported = cThread.justReported;
        uThread.justReporter = undefined;
        uThread.generator = thread.generator;
        Object.keys(thread).forEach((key) => {
            if (uThread[key] && uThread[key] != null) return;
            try {
              uThread[key] = structuredClone(thread[key]);
            } catch {
              uThread[key] = thread[key];
            }
          });
        thread.stopThisScript();
        thread = cThread;
        vm.runtime._stopThread(cThread);
