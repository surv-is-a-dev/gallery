...
(async function(Scratch) {
  ...
  const Explorer = (() => {
    ...
    /**
     * Binds an array of properties to an object.
     * @param {Array<PropertyKey | function>} props Array of properties to bind.
     * @param {Object} parent The parent containing all the properties.
     * @param {?Object} bindTo The object we (may) bind to.
     */
    const bindAll = (props, parent, bindTo) => {
      /** @type {Object} */
      bindTo ??= parent;
      for (const prop of props) {
        if (typeof prop === 'function') {
          if (isNullish(prop?.name)) {
            console.warn('Unable to bind a function property, add a name attribute to bind the function to the `bindTo` argument.');
            continue;
          }
          /** @type {function} */
          parent[prop.name] = prop.bind(bindTo);
          continue;
        }
        /** @type {function} */
        parent[prop] = parent[prop].bind(bindTo);
      }
    };
    const { Component, html } = Preact;
    /** Generate the header VDOM. @returns {object} */
    function AppHeader(props) {
      return html`<div class="header">
        <div class="headerItem titleItem">Custom modal (REAL NO CAP)</div>
        <div class="headerItem closeItem" onClick=${props.close}>
          <img alt="close button" draggable="false" src="data:image/svg+xml;base64,"></img>
        </div>
      </div>`;
    }
    class App extends Component {
      static exports = { AppHeader };
      constructor(props) {
        super(props);
        /** @type {Explorer} */
        this.explorer = this.props.explorer;
        /** @type {BFS} */
        this.fs = this.explorer.bfs_;
        bindAll([
          'close',
        ], this);
      }
      /** Closes the app. @returns {void} */
      close() {
        this.explorer.close();
      }
      /** Renders the app. @returns {object} */
      render() {
        return html`
          <${AppHeader} close=${this.close} />
          <div class="body">
            <span>Haiii!!!</span>
          </div>
        `;
    ...
    /** (Modal)Container selector. @type {string} @constant */
    const BSL = `div.BSEditBody`;
    /** (Modal)Header selector. @type {string} @constant */
    const BHL = `${BSL} > div.header`;
    /** (Modal)Body selector. @type {string} @constant */
    const MBS = `${BSL} > div.body`;
    class Explorer {
      static exports = { Preact, BFS, App, isNullish, bindAll };
      /** All of the styles for the entire app. @type {string} */
      static STYLE = `
        div.BSEditPortal {
          display: block;
        }
        div.BSEditPortal[closed] {
          display: none;
        }
      ...
      /**
       * Creates the body and does the initial setup and rendering.
       * @returns {void}
       * @private
       */
      createBody_() {
        this.body_ = document.createElement('div');
        this.body_.setAttribute('closed', '');
        this.body_.classList.add('BSEditPortal');
        const topBody = document.createElement('div');
        topBody.classList.add('BSEdit');
        topBody.addEventListener('click', () => this.app_.close());
        const style = document.createElement('style');
        style.textContent = Explorer.STYLE;
        this.div_ = document.createElement('div');
        this.div_.classList.add('BSEditBody');
        topBody.appendChild(style);
        topBody.appendChild(this.div_);
        this.body_.appendChild(topBody);
        document.body.appendChild(this.body_);
        Preact.render(this.app_ = html`<${App} explorer=${this} />`);
      }
      /**
       * Resets the app to be usable when we open it again.
       * @returns {void}
       * @private
       */
      resetDiv_() {}
      /** Closes the explorer. @returns {void} */
      close() {
        if (!this.app_ || !this.isOpen) return console.warn('Cannot close an unopened modal.');
        this.resetDiv_();
        this.body_.removeAttribute('opened');
        this.body_.setAttribute('closed', '');
      }
      /** Opens the explorer. @returns {void} */
      open() {
        if (!this.app_) this.createBody_();
        this.resetDiv_();
        this.body_.removeAttribute('closed');
        this.body_.setAttribute('opened', '');
      }
    }
    return Explorer;
  })();
