(function (Scratch) {
    ...
    class extension {
        ...
        registerColours(registerColors, colourMappings) {
            const myInfo = this.getInfo();

            // vvv this will make the colour object you need, this also create the color1-4 variable to. (takes 4 arguments in the form of hex code strings)
            const colours = colourMappings(myInfo.color1);
            // vvv this registers the colour and returns the "extension id" so that you can use it (just put it in the extensions array on all your blocks)
            const colourExtensionId = registerColours(myInfo.id, colours);
            ...
            this.blocks = this.blocks.map(/** @argument {Object} block */block => (typeof block !== 'object' ? block : {
                ...block,
                extensions: [...(block?.extensions ?? []), colourExtensionId]
            }));
        }
